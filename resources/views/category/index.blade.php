            ...
            @foreach($categories as $category)
                <div class="col-md-6 col-sm-12 col-lg-4 mb-4">
                    <div class="card hoverable">
                        <div class="rounded-circle mt-2  m-auto tables-icon bg-{{ $category->color }} d-flex justify-content-center align-content-center align-items-center">
                            <i class="{{ $category->logo }}"></i>
                            <span class="table-size {{ $category->products->count() > 0 ? 'bg-success' : 'bg-danger' }} " data-toggle="tooltip" data-placement="top" title="{{ $category->products->count() }} produkte">{{ $category->products->count() }}</span>
                        </div>
                        <div class="card-body">
